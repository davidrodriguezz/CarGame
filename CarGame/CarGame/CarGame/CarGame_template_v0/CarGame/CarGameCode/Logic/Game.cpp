...
void Game::update(bool& win){
	...
	for (int i = 0; i < obstacles.size(); i++) {
        if (obstacles[i]->isOutOfGame(car)) {
			delete obstacles[i];
            obstacles.erase(obstacles.begin()+i);
		}
		else if (CollisionRR(obstacles[i]->getCollider(), car->getCollider())) {
			car->lessLive();
			delete obstacles[i];
			obstacles.erase(obstacles.begin() + i);
			car->setNullVel();
...
//Las colisiones las comprobamos desde game (hecho)
...
//eliminar las piedras desde el update (hecho)
